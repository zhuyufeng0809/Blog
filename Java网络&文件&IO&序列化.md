...
一个微小但重要的差别是UDP协议保留了消息的边界信息。DatagramSocket的每-次receive()调用最多只能接收调用一次send()方法所发送的数据。而且，不同的receive()方法调用绝不会返回同一个send()方法调用所发送的数据
